...
- [~] 设置页改成「手机 OS 式二级菜单」—— **骨架 + 三组已跑通**（2026-09-24）。
      新增数据驱动的列表渲染器 `src/app/settings_menu.cpp`：加一项 = 加一行数组元素，
      不再是手摆绝对坐标。二级菜单 = **同一个屏 + 数据源栈**，不是每级建一个 Activity
      （否则 nav 表被撑爆 + 每页多一棵常驻对象树吃 DRAM）。
      三组：`显示与亮度`（亮度/息屏/桌面图标）、`浏览器设置`（缓存/下载/清理）、
      `系统设置`（时间源/自动校时/校准时间/固件 + 系统信息·后台管理·触摸校准三个入口）。
      📌 **分组口径**（改分组前先读 `settings.cpp` 头注释）：改了它，影响范围是
      **屏 / 浏览器 / 整机** —— 按这个归类；拿不准的先放系统设置，别为凑组硬塞。
