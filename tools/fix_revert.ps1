...
# 1) 构建
& $py -m platformio run -d $bdir -e esp32s3 2>&1 | Out-File "$tool\build_revert.log" -Encoding ascii
"BUILD_RC=$LASTEXITCODE"

# 2) 烧录（必须 DIO）
& $py $espy --chip esp32s3 --port COM7 --baud 921600 --before default_reset --after hard_reset `
  write_flash -z --flash_mode dio --flash_freq 80m --flash_size 16MB `
  0x0 "$arts\bootloader.bin" 0x8000 "$arts\partitions.bin" 0x10000 "$arts\firmware.bin" 2>&1 | Out-File "$tool\flash_revert.log" -Encoding ascii
