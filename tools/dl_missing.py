...
for (n, v), url in urls.items():
    ...
    if code_size and code_size[0] == '200':
        ...
        # try mirror2
        dlUrl2 = mirror2 + url if 'github.com' in url else url
        r2 = subprocess.run(['curl.exe', '-L', '-o', dest, dlUrl2, '--connect-timeout', '30', '--max-time', '300', '-s', '-w', '%{http_code} %{size_download}'], capture_output=True, text=True)
        code_size2 = r2.stdout.strip().split()
        if code_size2 and code_size2[0] == '200':
            print('  OK (mirror2): ' + str(int(code_size2[1])//1048576) + ' MB')
        else:
            print('  FAIL2: ' + (r2.stdout.strip() or r2.stderr.strip()[:60]))
